...
function SupplierLedger({ supplierId }: { supplierId: string }) {
  ...
  return (
    <Row gutter={16} style={{ marginTop: 16 }}>
      <Col span={8}><Card><Statistic title="Total Purchase Value" value={formatINR(data.totalPurchase)} /></Card></Col>
      <Col span={8}><Card><Statistic title="Total Paid" value={formatINR(data.totalPaid)} valueStyle={{ color: '#2e7d32' }} /></Card></Col>
      <Col span={8}><Card><Statistic title="Outstanding" value={formatINR(data.outstanding)} valueStyle={{ color: data.outstanding > 0 ? '#cf1322' : '#2e7d32' }} /></Card></Col>
    </Row>
  )
}
...
function CustomerLedger({ customerId }: { customerId: string }) {
  ...
  return (
    <Row gutter={16} style={{ marginTop: 16 }}>
      <Col span={8}><Card><Statistic title="Total Sale Value" value={formatINR(data.totalSale)} /></Card></Col>
      <Col span={8}><Card><Statistic title="Total Received" value={formatINR(data.totalReceived)} valueStyle={{ color: '#2e7d32' }} /></Card></Col>
      <Col span={8}><Card><Statistic title="Outstanding" value={formatINR(data.outstanding)} valueStyle={{ color: data.outstanding > 0 ? '#cf1322' : '#2e7d32' }} /></Card></Col>
    </Row>
  )
}
...
export default function PaymentsPage() {
  ...
  const spColumns = [
    ...
    { title: 'Date', dataIndex: 'paymentDate', key: 'date', render: (v: string) => dayjs(v).format('DD MMM YYYY') },
  ...
  const crColumns = [
    ...
    { title: 'Date', dataIndex: 'receiptDate', key: 'date', render: (v: string) => dayjs(v).format('DD MMM YYYY') },
  ...
  const paymentFormItems = (namePrefix: 'supplier' | 'customer') => (
    <>
      <Form.Item label={namePrefix === 'supplier' ? 'Supplier' : 'Customer'} name={namePrefix === 'supplier' ? 'supplierId' : 'customerId'} rules={[{ required: true }]}>
        <Select showSearch optionFilterProp="label"
          options={(namePrefix === 'supplier' ? suppliers : customers).map((s: any) => ({ value: s.id, label: s.name }))}
        />
      </Form.Item>
      <Form.Item label="Date" name={namePrefix === 'supplier' ? 'paymentDate' : 'receiptDate'} rules={[{ required: true }]}>
  ...
  return (
    <div>
      <Typography.Title level={4} style={{ marginBottom: 16 }}>Payments & Receipts</Typography.Title>
      <Tabs
        items={[
          {
            ...
            children: (
              <div>
                ...
                <Table dataSource={spayments} columns={spColumns} rowKey="id" loading={spLoading} style={{ marginTop: 16 }} />
              </div>
            )
          },
          {
            ...
            children: (
              <div>
                ...
                <Table dataSource={receipts} columns={crColumns} rowKey="id" loading={crLoading} style={{ marginTop: 16 }} />
              </div>
            )
          }
        ]}
      ...
      <Modal title="Record Supplier Payment" open={supplierOpen} onOk={saveSP} onCancel={() => setSupplierOpen(false)}>
        <Form form={spForm} layout="vertical">{paymentFormItems('supplier')}</Form>
      </Modal>
      <Modal title="Record Customer Receipt" open={customerOpen} onOk={saveCR} onCancel={() => setCustomerOpen(false)}>
        <Form form={crForm} layout="vertical">{paymentFormItems('customer')}</Form>
      </Modal>
    </div>
  )
}
